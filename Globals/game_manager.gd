...
# production_rate and the count assigned. When at least one whole unit of work
# has accumulated, debris is processed: region.trash decrements by 1, the
# region's drop table picks a resource via region.return_resource(), and the
# robot's resource_return (which can be fractional, e.g. Atomizer 0.4 or
# Recycler 2.5) accumulates into a per-(region, resource) carry pool that
# drains whole integer amounts to GlobalResources.got_resource().
#
# Pollution is tracked on a separate per-(region, robot) carry pool that
# accumulates count * robot.pollution_effect * delta — independent of the
...
# pollution_effect (it's a pollution-system tech).
...
func _process(delta: float) -> void:
	if production_paused:
		return
	var solar_system: SolarSystemData = GlobalResources.solar_system
	...
	for planet in solar_system.planets:
		...
		for region in planet.regions:
			if region.locked:
				continue
			if region.assigned_robots.is_empty() and region.building_workers.is_empty() and (pollution_decay == 0 or region.pollution <= 0):
				continue
...
func _tick_region(region: RegionData, planet: PlanetData, delta: float, global_mult: float, cleaner_scalar: float, pollution_decay: int) -> Dictionary:
	...
	if pollution_decay != 0 and region.pollution > 0:
		var decayed: int = clamp(region.pollution + pollution_decay, 0, region.max_pollution)
	...
	for robot in region.assigned_robots:
		...
		progress += float(count) * float(robot.production_rate) * pollution_mult * global_mult * delta
		...
		while progress >= 1.0 and region.trash > 0:
			...
			var res: String = region.return_resource()
			if res == "":
				continue

			var key := res.to_lower()
			var pooled: float = region_carry.get(key, 0.0) + float(robot.resource_return)
			var whole: int = int(floor(pooled))
			if whole > 0:
				for _i in range(whole):
					var deposited_into = region.deposit_resource(key)
		...
		var effective_rate: float = robot.pollution_effect
		...
		if effective_rate != 0.0:
			...
			if carry >= 1.0:
				...
				carry -= float(drain)
			if drain != 0:
				var new_pollution: int = clamp(region.pollution + drain, 0, region.max_pollution)
	...
	for b in region.building_workers:
		...
		while progress >= 1.0:
			...
			if moved.is_empty():
				...
				break
			for res in moved:
				GlobalResources.got_resource(str(res), int(moved[res]))
