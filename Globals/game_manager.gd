...
#
# Pollution effects are intentionally not applied yet (see plan).
...
func _process(delta: float) -> void:
	...
	for planet in solar_system.planets:
		...
		for region in planet.regions:
			...
			var region_changed := _tick_region(region, delta)
			if region_changed:
...
func _tick_region(region: RegionData, delta: float) -> bool:
	...
	for robot in region.assignedRobots:
		...
		progress += float(count) * float(robot.productionRate) * delta
